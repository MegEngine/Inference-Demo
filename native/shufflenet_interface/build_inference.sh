...
NATIVE_SRC_DIR=${CUR_DIR}/
APK_SRC_DIR=${CUR_DIR}/Camera2Basic/
...
if (( ($BUILD_CMD & 0x10) != 0 )); then
    ...
    if (( ($BUILD_CMD & 0x2) != 0 )); then
        echo "-----------------install bin ----------------------"
        test_android_devices &&
        adb push ${NATIVE_SRC_DIR}/install/shufflenet_loadrun /data/local/tmp/mge_tests
    fi
fi
